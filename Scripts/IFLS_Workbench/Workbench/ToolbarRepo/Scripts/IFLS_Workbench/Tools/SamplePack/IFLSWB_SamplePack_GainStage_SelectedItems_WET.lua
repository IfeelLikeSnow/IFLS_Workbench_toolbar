-- @description IFLS Workbench - Workbench/ToolbarRepo/Scripts/IFLS_Workbench/Tools/SamplePack/IFLSWB_SamplePack_GainStage_SelectedItems_WET.lua
-- @version 0.63.0
...
local TARGET_DBTP = -1.0
local NORM_TO = 3 -- true peak
...
local function main()
  ...
  if n == 0 then
    r.MB("Select audio items to gain-stage.", "IFLSWB GainStage WET", 0)
    return
  end
  ...
  for i=0,n-1 do
    ...
    if take and not r.TakeIsMIDI(take) then
      ...
      if src then
        ...
        local gain_db = r.CalculateNormalization(src, NORM_TO, TARGET_DBTP, a, b) or 0
...
r.Undo_BeginBlock()
main()
r.Undo_EndBlock("IFLS WB: SamplePack GainStage WET (TP -1)", -1)
