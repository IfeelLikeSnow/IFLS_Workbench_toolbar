-- @description IFLS Workbench - Workbench/ToolbarRepo/Scripts/IFLS_Workbench/Tools/SamplePack/IFLSWB_SamplePack_Export_SelectedItems_DRY.lua
-- @version 0.63.0
...
local function main()
  if r.CountSelectedMediaItems(0) == 0 then
    r.MB("Select items to export.", "IFLSWB Export DRY", 0)
    return
  end

  -- Output folder: <project>/Rendered/SamplePack/DRY
  ...
  local out_dir = proj_path .. sep .. "Rendered" .. sep .. "SamplePack" .. sep .. "DRY"
  ensure_dir(out_dir)
  ...
  local SOURCE_MASK = 1+2+8+32+64+128
  local new_settings = (old_settings & (~SOURCE_MASK)) | 32 -- 32 = selected media items (dry)
...
r.Undo_BeginBlock()
main()
r.Undo_EndBlock("IFLS WB: Export DRY (selected media items)", -1)
