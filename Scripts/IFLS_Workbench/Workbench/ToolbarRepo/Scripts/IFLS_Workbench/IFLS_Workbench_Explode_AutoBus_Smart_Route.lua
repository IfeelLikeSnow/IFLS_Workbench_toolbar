-- @description IFLS Workbench - Workbench/ToolbarRepo/Scripts/IFLS_Workbench/IFLS_Workbench_Explode_AutoBus_Smart_Route.lua
-- @version 0.63.0
...
local FX_BUS_NAME       = "IFLSWB FX Bus"
local COLORING_BUS_NAME = "IFLSWB Coloring Bus"
local MASTER_BUS_NAME   = "IFLSWB Master Bus"

-- Wenn du nach dem Anlegen der Busse automatisch deine IFLSWB-Selector-Scripts
-- starten willst, kannst du hier die Action-Kommandos eintragen.
-- Beispiel: local CMD_IFLSWB_MASTER_SELECTOR = r.NamedCommandLookup("_IFLSWB_MASTERBUS_SELECTOR")
local CMD_IFLSWB_MASTER_SELECTOR   = 0 -- 0 = aus
...
local function ensure_all_busses()
  ...
  dbg(string.format("[IFLSWB] FXBus=%d (new=%s) | Coloring=%d (new=%s) | MasterBus=%d (new=%s)",
    fx_idx, tostring(fx_new), col_idx, tostring(col_new), master_idx, tostring(mnew)))

  if CMD_IFLSWB_MASTER_SELECTOR ~= 0 then
    r.Main_OnCommand(CMD_IFLSWB_MASTER_SELECTOR, 0)
  end
  if CMD_IFLSWB_FXBUS_SELECTOR ~= 0 then
    r.Main_OnCommand(CMD_IFLSWB_FXBUS_SELECTOR, 0)
  end
  if CMD_IFLSWB_COLORING_SELECTOR ~= 0 then
    r.Main_OnCommand(CMD_IFLSWB_COLORING_SELECTOR, 0)
  end
...
local function main()
  ...
  if num_sel_tr > 0 then
    ...
    if has_multi and (CMD_EXPLODE_MULTI_CANDIDATES and #CMD_EXPLODE_MULTI_CANDIDATES > 0) then
      for _, cmd in ipairs(CMD_EXPLODE_MULTI_CANDIDATES) do r.Main_OnCommand(cmd, 0) end
    end
  end

  route_sources_to_busses(fx_tr, col_tr, master_tr)

  r.Undo_EndBlock("IFLSWB Explode AutoBus Smart + Route", -1)
end
