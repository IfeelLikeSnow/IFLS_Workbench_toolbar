...
local wb_root = r.GetResourcePath() .. "/Scripts/IFLS_Workbench"
local LibPath = dofile(wb_root .. "/Workbench/FB01/Lib/ifls_fb01_library_path.lua")
...
local function discover()
  ...
  local internal_manifest = wb_root .. "/Workbench/FB01/PatchLibrary/Patches/manifest.json"
  if file_exists(internal_manifest) then
    local internal_root = wb_root .. "/Workbench/FB01/PatchLibrary/Patches"
...
local function send_syx_file(path)
  ...
  dofile(wb_root .. "/Workbench/FB01/Pack_v8/Scripts/IFLS_FB01_Replay_SYX_File_FromPath.lua")
end
  ...
  if visible then
    ...
    if selected>0 and items[selected] then
      ...
      if r.ImGui_Button(ctx, "Safe Audition (backup->audition->revert)", -1, 0) then
        dofile(root .. "/Workbench/FB01/Tools/IFLS_FB01_Safe_Audition_Backup.lua")
      end
