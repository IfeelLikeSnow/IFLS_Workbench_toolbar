...
local function get_db_path_multi_ucs()
  ...
  dir = join_path(dir, "IFLSWB_SampleDB")
  return dir, join_path(dir, "IFLSWB_SampleDB_Multi_UCS.json")
end
...
local function do_explode_multichannel(opt_enable_mapping, opt_enable_sampledb)
  ...
  r.Undo_EndBlock("IFLS Workbench PolyWAV Toolbox – Explode + ZoomF6 + SampleDB", -1)
end
...
local function ensure_imgui()
  ...
  ctx = r.ImGui_CreateContext("IFLS Workbench PolyWAV Toolbox")
  return ctx
end
...
local function loop()
  ...
  local visible, open = r.ImGui_Begin(ctx, "IFLS Workbench PolyWAV Toolbox – ZoomF6", true)
  ...
  if visible then
    ...
    -- Haupt-Button: Explode + Mapping + DB
    ...
    r.ImGui_Text(ctx, "→ nutzt REAPER-Action + IFLSWB-Mapping/Bridge")
    ...
    local _ifls_child_border_flags = nil
    if r.ImGui_ChildFlags_Border then
      _ifls_child_border_flags = r.ImGui_ChildFlags_Border()
      r.ImGui_BeginChild(ctx, "IFLSWB_PolyWAV_Log", -1, 100, _ifls_child_border_flags)
    else
      -- Older ReaImGui signature used a boolean border argument
